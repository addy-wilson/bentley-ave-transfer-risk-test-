    ...
    for month in SEASON_MONTHS:
        ...
        for entry in data.get("gameDates", []):
            if entry.get("games", 0) > 0:
                d = entry.get("contest_date")  # format: MM-DD-YYYY
                if d:
                    parts = d.split("-")
                    if len(parts) == 3:
    ...
    for team_box in data.get("teamBoxscore", []):
        ...
        for player in team_box.get("playerStats", []):
            row = {
                ...
                "season": meta["game_date"].split("-")[-1] if meta["game_date"] else "",
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 4 — Determining transfer/left status
# ─────────────────────────────────────────────────────────────────────────────

def add_transfer_labels(player_df: pd.DataFrame) -> pd.DataFrame:
    """
    Infers transfer/left status by checking if a player appears
    on a different team the following season, or disappears entirely.
    
    Adds two columns:
      - transferred: 1 if player moved to a different team next season
      - left_program: 1 if player does not appear at all next season
    """
    # Sort so we can look forward
    df = player_df.sort_values(["player_first", "player_last", "season"]).copy()
    df["season"] = df["season"].astype(int)

    # Create a lookup of player -> next season's team
    next_season = df[["player_first", "player_last", "team_seo", "season"]].copy()
    next_season["season"] = next_season["season"] - 1  # shift back to match current
    next_season = next_season.rename(columns={"team_seo": "next_team_seo"})

    df = df.merge(
        next_season,
        on=["player_first", "player_last", "season"],
        how="left"
    )

    # Left program = no record next season at all
    df["left_program"] = df["next_team_seo"].isna().astype(int)

    # Transferred = appeared next season but on a different team
    df["transferred"] = (
        df["next_team_seo"].notna() &
        (df["next_team_seo"] != df["team_seo"])
    ).astype(int)

    df.drop(columns=["next_team_seo"], inplace=True)
    return df
